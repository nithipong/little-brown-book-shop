<template>
  <transition-custom>
    <div class="product-list" v-show="products.length > 0">
      ...
      name="catd-product"
      :css="false" @before-enter="beforeEnter" @enter="enter" @leave="leave">
      <div v-for="(product, index) in products" :key="product.id" :data-index="index" class="column is-6">
        <div class="card">
        <div class="columns is-mobile is-gapless">
          <div class="column is-narrow" style="min-width: 150px">
...
export default {
  ...
  methods: {
    ...mapActions(['initProducts', 'addToShoppingCart', 'updateProductQty', 'removeProduct']),
    ...
    addToCart(product) {
      ...
      if (self.shoppingCart.products.length > 0) {
        let indexProductExist = self.shoppingCart.products.findIndex(item => item.id === product.id)
